  ...
  describe '#call' do
      ...
      it 'records exception' do
        Thread.new do
          expect(Traxor::Metric).to(
            receive(:count).with(Traxor::Sidekiq::COUNT_METRIC, 1, tags)
          )
          expect(Traxor::Metric).to(
            receive(:count).with(Traxor::Sidekiq::EXCEPTION_METRIC, 1, tags)
          )
          begin
    ...
    context 'when no exception' do
      it 'records metrics' do
        Thread.new do
          expect(Traxor::Metric).to(
            receive(:measure).with(Traxor::Sidekiq::DURATION_METRIC, any_args, tags)
          )
          expect(Traxor::Metric).to(
            receive(:count).with(Traxor::Sidekiq::COUNT_METRIC, 1, tags)
          )
